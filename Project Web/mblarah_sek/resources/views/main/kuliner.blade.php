    ...
    <div class="row">
        ...
        <div class="col-md-4 col-lg-6 col-sm-6  pb-5 pt-2">
            <div class="card border-0 shadow rounded-3 overflow-hidden fit-content w-100" style="border-radius: 20px;">
                <img src="{{ Storage::url($food->gambar1)}}" style="height:20rem;">
                <h4 class="pl-4"><b>{{("$food->nama")}}</b></h4>
                <p class="pl-4">{{("$food->alamat")}}</p>
                <a class="nav-link" href="{{route('detail_kuliner',$food->id)}}"> <button class="btn btn-secondary">Detail</button></a>
            </div>
        </div>
        @endforeach
    </div>
